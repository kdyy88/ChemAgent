...
export function getSdfSplitDownloadUrl(resultId: string): string {
  return `${BASE_URL}/api/v1/babel/sdf-split-download?result_id=${encodeURIComponent(resultId)}`
}

export function getSdfMergeDownloadUrl(resultId: string): string {
  return `${BASE_URL}/api/v1/babel/sdf-merge-download?result_id=${encodeURIComponent(resultId)}`
}
...
export async function fetchScratchpad(
  ...
  const url = `${BASE_URL}/api/v1/scratchpad/${encodeURIComponent(scratchpadId)}?session_id=${encodeURIComponent(sessionId)}&sub_thread_id=${encodeURIComponent(subThreadId)}`
