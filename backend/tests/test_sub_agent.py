...
from app.skills.loader import load_required_skill_markdown
...
def all_tool_names() -> set[str]:
    ...
    from app.tools.catalog import ALL_CHEM_TOOLS
...
class TestWebSearchTool:
    async def test_web_search_uses_tavily(self) -> None:
        from app.tools.pubchem.search import tool_web_search
        ...
        with patch.dict("os.environ", {"TAVILY_API_KEY": "tvly-test-key"}, clear=False):
            with patch("app.tools.pubchem.search.TavilyClient") as client_cls:
...
class TestSubAgentToolHelpers:
    ...
    def test_resolve_runtime_ids_uses_plan_id_for_plan_mode(self) -> None:
        from app.agents.sub_agents.tool import _resolve_runtime_ids
    ...
    def test_normalize_artifact_pointers_prefers_explicit_then_parent(self) -> None:
        from app.agents.sub_agents.tool import _normalize_artifact_pointers
    ...
    def test_normalize_delegation_keeps_short_context_inline(self) -> None:
        ...
        from app.agents.sub_agents.tool import _normalize_delegation_payload
    ...
    def test_normalize_delegation_writes_long_context_to_scratchpad(self) -> None:
        ...
        with patch("app.agents.sub_agents.tool.create_scratchpad_entry") as create_entry:
            delegation = _normalize_delegation_payload(
                mode="explore",
                task="提取共同骨架特征",
    ...
    def test_preflight_rejects_explore_design_with_forbid_new_smiles(self) -> None:
        from app.agents.sub_agents.tool import (
    ...
    def test_tool_schema_excludes_always_denied(self) -> None:
        ...
        from app.agents.sub_agents.tool import tool_run_sub_agent
    ...
    def test_infer_required_mode(self) -> None:
        from app.agents.sub_agents.tool import _infer_required_mode
...
class TestRunSubAgentToolIntegration:
    ...
    async def test_plan_mode_returns_pending_approval_status(self) -> None:
        ...
        with (
            patch("app.agents.sub_agents.tool.build_sub_agent_graph", return_value=mock_graph),
            patch("app.agents.main_agent.runtime.get_checkpointer", return_value=mock_checkpointer),
            patch("app.agents.sub_agents.tool.create_scratchpad_entry", return_value=fake_ref),
        ):
    ...
    async def test_run_sub_agent_returns_produced_artifacts_and_suggested_smiles(self) -> None:
        ...
        with (
            ...
            patch("app.agents.sub_agents.tool.create_scratchpad_entry", return_value=fake_ref),
            patch(
                "app.agents.sub_agents.tool._lg_get_config",
                create=True,
                return_value={
                    "configurable": {
    ...
    async def test_structured_completion_generates_report_content_without_placeholder(self) -> None:
        ...
        with (
            patch("app.agents.sub_agents.tool.build_sub_agent_graph", return_value=mock_graph),
            patch("app.agents.main_agent.runtime.get_checkpointer", return_value=MagicMock()),
            patch("app.agents.sub_agents.tool.create_scratchpad_entry", create_entry),
        ):
            result_str = await tool_run_sub_agent.ainvoke(
                {
                    "mode": "explore",
    ...
    async def test_timeout_returns_timeout_status(self) -> None:
        ...
        with (
            patch("app.agents.sub_agents.tool.build_sub_agent_graph", return_value=mock_graph),
            patch("app.agents.main_agent.runtime.get_checkpointer", return_value=mock_checkpointer),
            patch("app.agents.sub_agents.tool._SUB_AGENT_TIMEOUT", 0.05),
        ):
            result_str = await tool_run_sub_agent.ainvoke(
                {
                    "mode": "explore",
    ...
    async def test_custom_invalid_tool_returns_error(self) -> None:
        ...
        with patch("app.agents.main_agent.runtime.get_checkpointer", return_value=mock_checkpointer):
            result_str = await tool_run_sub_agent.ainvoke(
                {
                    "mode": "custom",
                    "task": "Some task",
