...
def build_llm(structured_schema: type | None = None, model: str | None = None) -> Any:
    """Build a ChatOpenAI instance from the active LLM config.

    Optionally wraps it with ``with_structured_output()`` for schema-constrained
    generation (planner, router nodes).
    """
