...
from app.agents.config import get_fast_llm_config, get_llm_config
...
def parse_routing_decision(text: str) -> dict:
    ...
    if not isinstance(data, dict):
        # Strategy 2: unwrap fenced code block if present
        data = None
...
_SYNTHESIS_SYSTEM_MESSAGE = """你是首席化学科学家，也是本次对话的主持人。各专家助手已完成各自的调研与工具任务，现在由你向用户给出最终的综合答案。
...
def create_manager() -> AssistantAgent:
    ...
    manager = create_assistant_agent(
        name="Manager",
        system_message=_SYNTHESIS_SYSTEM_MESSAGE,
