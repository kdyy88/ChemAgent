...
Routes (all under the /api/v1 prefix added in main.py):
    POST /api/v1/babel/convert         — Tool 1: universal format converter
    POST /api/v1/babel/conformer3d     — Tool 2: 3D conformer builder (+ energy)
    POST /api/v1/babel/pdbqt           — Tool 3: docking PDBQT prep
    POST /api/v1/babel/properties      — T7: molecular properties
    GET  /api/v1/babel/formats         — Utility: supported format list
    POST /api/v1/babel/partial-charges — F2: atom partial charge analysis
    POST /api/v1/babel/sdf-split       — F3: split multi-mol SDF → ZIP
    POST /api/v1/babel/sdf-merge       — F3: merge multiple SDF → single SDF
